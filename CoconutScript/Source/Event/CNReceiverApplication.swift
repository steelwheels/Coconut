...
public class CNReceiverApplication: CNRemoteApplication
{
	...
	static private let descriptionForError: [Int:String] = [
		// OS errors
		-34: "Disk is full.",
	...
	/*
	public struct EventDescriptorType {
		var 	eventClass:		AEKeyword
		var	eventID:		AEEventID
		var	parameters:		Dictionary<AEKeyword, NSAppleEventDescriptor>
		var	directParameter:	NSAppleEventDescriptor?

		public init(eventClass evtcls: AEKeyword, eventId evtid: AEEventID){
			eventClass	= evtcls
			eventID		= evtid
			parameters	= [:]
			directParameter	= nil
	...
	public func sendEvent(eventDescriptor desc: CNAppleEventDescriptor) -> Bool {
		let event = desc.toEvent(target: mDescriptor)
		NSLog("event \(event.description)")
		return sendEventDescriptor(eventDescriptor: event)
	}

	private func sendEventDescriptor(eventDescriptor event: NSAppleEventDescriptor) -> Bool {
		do {
			let result = try event.sendEvent(options: .waitForReply, timeout: 1.0)
			if let err = result.paramDescriptor(forKeyword: CNReceiverApplication.keyErrorNumber) {
				let code = Int(err.int32Value)
				if let msg = CNReceiverApplication.descriptionForError[code] {
					NSLog("Error code: \(code) \(msg)")
				} else {
					NSLog("Error code: \(code) Unknown")
				}
				return true
			} else {
				NSLog("No error code in received event")
				return false
			}
		} catch {
