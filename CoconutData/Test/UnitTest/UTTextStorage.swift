...
public func testTextStorage(console cons: CNConsole) -> Bool
{
	...
	for ecode in ecodes {
		idx = execute(string: str, index: idx, escapeCode: ecode, terminalInfo: tinfo, console: cons)
	}

	return true
}

private func execute(string str: NSMutableAttributedString, index idx: Int, escapeCode ecode: CNEscapeCode, terminalInfo tinfo: CNTerminalInfo, console cons: CNConsole) -> Int
{
	if let result = str.execute(index: idx, terminalInfo: tinfo, escapeCode: ecode) {
