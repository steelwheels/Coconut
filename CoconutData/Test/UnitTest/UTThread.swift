...
public class UTNestedThread: CNThread {
	...
	public init(queue disque: DispatchQueue, input instrm: CNFileStream, output outstrm: CNFileStream, error errstrm: CNFileStream, environment env: CNEnvironment, count cnt: Int) {
		mCount = cnt
...
public func testThread(console cons: CNFileConsole) -> Bool
{
	...
	let result0 = testSimpleThread(queue: queue, environment: env, console: cons)
	let result1 = testNestedThread(queue: queue, environment: env, console: cons)
	return result0 && result1
}

private func testSimpleThread(queue disque: DispatchQueue, environment env: CNEnvironment, console cons: CNFileConsole) -> Bool
{
...
private func testNestedThread(queue disque: DispatchQueue, environment env: CNEnvironment, console cons: CNFileConsole) -> Bool
{
